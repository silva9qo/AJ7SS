...
const SUSPICIOUS_TLDS = [
  ".store", ".netlify.app", ".netlify", ".xyz", ".pw",
...
async function analyze(entries) {
  ...
  let netEntries = entries.filter(e => {
    ...
    // FILTRAGEM AGRESSIVA: Whitelist, IPs numéricos e domínios seguros
    if (CLEAN_WHITELIST.some(w => domain.includes(w.toLowerCase()))) return false;
    if (/^(\d{1,3}\.){3}\d{1,3}$/.test(domain)) return false;
    if (domain.includes(":") && (domain.match(/[a-fA-F0-9]/g) || []).length > 4) return false;
    let safe = ["apple.com","icloud.com","google.com","googleapis.com","gstatic.com","mzstatic.com","418bet.site"];
  ...
  for (let i = 0; i < allDomains.length; i += CHUNK) {
    ...
    for (let j = 0; j < results.length; j++) {
      ...
      // BLOQUEIO TOTAL DE IPS NUMÉRICOS E DOMÍNIOS ESPECÍFICOS
      let isIPv4 = /^(\d{1,3}\.){3}\d{1,3}$/.test(domain);
      let isIPv6 = domain.includes(":") && (domain.match(/[a-fA-F0-9]/g) || []).length > 4;
      if (isIPv4 || isIPv6 || domain.includes("418bet.site")) continue;
