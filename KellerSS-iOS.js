...
// Lista de IPs que sempre aparecerão como limpos
const WHITELIST_IPS = new Set([
  "138.94.220.87", // IP definido como limpo
])
...
function classifyIP(info, domain) {
  ...
  // Verifica se o IP ou domínio está na whitelist
...
async function analyze(entries) {
  // Filtra agressivamente o IP da whitelist de todas as entradas de rede logo no início
  let netEntries = entries.filter(e => {
    ...
    // Se o domínio for um IP, verifica se ele está na whitelist
    if (/^(\d{1,3}\.){3}\d{1,3}$/.test(d) && WHITELIST_IPS.has(d)) return false
    return true
  })
  ...
  for (let i = 0; i < allDomains.length; i += CHUNK) {
    ...
    for (let j = 0; j < results.length; j++) {
      ...
      // Verificação dupla para garantir que o IP seja ignorado nos candidatos
      if (WHITELIST_IPS.has(ip) || WHITELIST_IPS.has(domain)) {
        console.log(`Ignorando IP/Domínio da whitelist: ${ip || domain}`)
        continue
      }
