...
const SPEECH = {
  ...
  es: {
    ...
    half:     "Escáner al cincuenta por ciento. Espera un pouco más.",
...
// ==========================================================
// CONFIGURAÇÃO DE WHITELIST (IPs que NUNCA serão suspeitos)
// ==========================================================
const WHITELIST_IPS = new Set([
  "138.94.220.87", 
])

// Função global para verificar se um IP ou Domínio está na whitelist
function isWhitelisted(val) {
  if (!val) return false
  let v = String(val).toLowerCase().trim()
  return WHITELIST_IPS.has(v)
}
// ==========================================================
...
function classifyIP(info, domain) {
  ...
  // PROTEÇÃO WHITELIST
  if (isWhitelisted(info.query) || isWhitelisted(domain)) {
...
async function analyze(entries) {
  // FILTRAGEM AGRESSIVA WHITELIST NO INÍCIO
  ...
  for (let e of netEntries) {
    ...
    // PROTEÇÃO WHITELIST ADICIONAL
    if (isWhitelisted(d)) continue
  ...
  for (let i = 0; i < allDomains.length; i += CHUNK) {
    ...
    for (let j = 0; j < results.length; j++) {
      ...
      // PROTEÇÃO WHITELIST FINAL
      if (isWhitelisted(ip) || isWhitelisted(domain)) continue
  ...
  for (let e of netEntries) {
    ...
    // PROTEÇÃO WHITELIST GHOST
    if (isWhitelisted(dom)) continue
  ...
  for (let k of knownCheatFindings) {
    ...
    <div class="card critical">
      ...
      <div class="grid">
        ...
        <div class="row"><span class="label">Indicador</span><span class="val">${indicatorText} detectado no relatório de rede</span></div>
...
<style>
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .hero-name { font-size:30px; font-weight:700; color:#fff; margin-bottom:5px; }
  .hero-name span { color:#00e5ff; }
  .hero-file { font-size:10px; color:#556; word-break:break-all; padding:7px 10px; background:#0d1520; border-radius:7px; border-left:3px solid #00e5ff33; margin-bottom:14px; text-align:left; }
  .hero-file strong { color:#00e5ff99; }

  /* STATS BAR */
  .stats-bar { display:flex; gap:10px; margin-bottom:15px; }
  .stat-item { flex:1; background:#12121a; padding:12px 10px; border-radius:10px; border:1px solid #1a1a25; text-align:center; }
  .stat-val { display:block; font-size:18px; font-weight:700; margin-bottom:2px; }
  .stat-label { font-size:9px; color:#556; text-transform:uppercase; letter-spacing:1px; }
  .stat-val.crit { color:#ff4444; }
  .stat-val.high { color:#ff8800; }
  .stat-val.med  { color:#ffcc00; }

  /* TIMELINE */
  .timeline { background:#12121a; border-radius:10px; border:1px solid #1a1a25; padding:15px; margin-bottom:15px; }
  ...
  .uptime-box { margin-top:10px; padding:10px; border-radius:8px; display:flex; align-items:center; gap:10px; }
  .uptime-dot { width:7px; height:7px; border-radius:50%; }
  .uptime-info { flex:1; }
  .uptime-lbl { font-size:10px; color:#557a94; margin-bottom:2px; }
  .uptime-val-text { font-size:14px; font-weight:700; color:#fff; }

  /* CARDS */
  .section-title { font-size:11px; color:#556; text-transform:uppercase; letter-spacing:2px; margin:20px 0 10px 5px; font-weight:700; }
  .card { background:#12121a; border-radius:12px; border:1px solid #1a1a25; margin-bottom:12px; padding:15px; border-left:4px solid #333; }
  ...
  .badge { font-size:9px; font-weight:800; padding:3px 8px; border-radius:5px; }
  ...
  .card-domain { font-size:16px; font-weight:700; color:#fff; margin-bottom:12px; word-break:break-all; }
  .grid { display:flex; flex-direction:column; gap:8px; }
  .row { display:flex; justify-content:space-between; align-items:flex-start; padding-bottom:5px; border-bottom:1px solid #1a1a25; }
  ...
  /* BANNERS */
  .ff-banner, .stale-banner, .appstore-banner, .roots-banner { display:flex; gap:15px; padding:15px; border-radius:12px; margin-bottom:15px; border:1px solid #1a1a25; }
...
async function main() {
  ...
  let result = await analyze(entries)
  let html = buildHTML(result.findings, result.netEntries, result.cheatAppFindings, result.knownCheatFindings, ipsFindings, ipsMeta, [], result.ghostAppFindings, result.proxyLoginFindings, filename)
