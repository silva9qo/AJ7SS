...
function analyzeIps(parsed) {
  ...
  for (let e of entries) {
    ...
    let isWhitelisted = WHITELIST.some(w => {
      let lowW = w.toLowerCase();
      return lowW !== "" && lowW !== "seu_ip_aqui" && bid.toLowerCase().includes(lowW);
    });
...
async function analyze(entries) {
  ...
  // Esta parte remove QUALQUER conexão que combine com os termos da whitelist
  // impedindo que sejam checadas contra listas de ASNs, IPs suspeitos ou infra de cheat.
  let netEntries = entries.filter(e => {
    if (e.type !== "networkActivity") return false
    let domain = (e.domain || "").toLowerCase()
    let isWhitelisted = WHITELIST.some(w => {
      let lowW = w.toLowerCase()
      return lowW !== "" && lowW !== "seu_ip_aqui" && domain.includes(lowW)
    })
    return !isWhitelisted
  })

  // Também garante que a infraestrutura de cheat conhecida seja filtrada pela whitelist
  // se o usuário adicionou algum desses termos na lista.
  const CLEAN_WHITELIST = WHITELIST.filter(w => w.toLowerCase() !== "" && w.toLowerCase() !== "seu_ip_aqui");
  let domainBundles = {}
